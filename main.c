...
//------------------------------------------------------------------------------------
void Calculate_average_mark ( node *pStud ) {//Gets node* head of forward list and calculates average mark for each node
    double sum = 0 ;
...
void Print_student ( node *pStud ) {
 //prints formatted information about the student under the pointer
...
void Scan_students_from_file ( FILE *source, node **StudList ) {
 //takes pointer of .txt file and second pointer of the forward list head.
 //Scans students from file into the list up to the end
    while ( ! feof ( source ) ) {
        node *p = NULL ;
